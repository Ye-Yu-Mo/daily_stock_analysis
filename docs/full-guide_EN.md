...
> Compatibility note (Issue #1815): `MARKET_REVIEW_REGION=cn|hk|us|jp|kr|tw|both` only expands the market set used by market review; `jp`/`kr` are for recap scope and do not open JP/KR for Market Light alerts (TW is supported for Market Light alerts at index-only granularity; see [market support boundaries](market-support.md)).
